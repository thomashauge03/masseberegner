...
/* Hvordan en flis tolkes. Star i navnet pa mellomlagrede filer, sa gamle
   ...
   3: og fliser som er DELVIS null der nullflaten grenser mot høyt terreng –
      dekningskanten, ikke havflaten. Se `nullflateKant`. Uten bumpen her ble
      de seksten forgiftede flisene i mellomlageret liggende og bli levert i
      det uendelige. */
const FLIS_TOLKNING = 3;
...
async function hentFlis(sr, tx, ty, res, modell) {
  ...
  const jobb = (async () => {
    ...
    else if (d.length) nullflateKant(d, px);
...
/**
 * En flis kan være HALVT utenfor dekningen, og da holder ikke «bare null».
 *
 * Testen over fanger flisen som er null over alt. Men en flis som ligger PÅ
 * dekningskanten har ekte data i den ene enden og bitkorrekte nuller i den
 * andre, og den slapp gjennom med nullene som helt vanlige koter. Målt på
 * prosjektets egen flisbuffer: 17 av 270 fliser har både null og ekte data.
 * Seksten av dem er dekningskanter i Finnmark der nabocellene til nullene
 * ligger på 680–790 meter – altså et stup på sju hundre meter tvers over en
 * cellegrense. Utslaget er ikke subtilt: en fyllingsskråning som treffer
 * nullflaten marsjerer utover og lander aldri.
 *
 * Men den syttende flisen er en EKTE STRANDLINJE, og der er nullene riktige:
 * DTM-en modellerer havflaten som null, og nabocellene ligger på 0,02 til
 * 2,64 meter. Å slette hver null ville tatt 47 128 gyldige celler i den ene
 * flisen alene – det er nettopp det tilfellet kommentaren over verner om.
 *
 * Skillet er ikke om det finnes nuller, men hvor høyt det spretter ved siden
 * av dem. Målt 10-persentil av naboene: 0,02 m for strandlinja, 678–766 m for
 * de seksten kantene. Grensa på fem meter ligger to størrelsesordener fra
 * begge, så den avgjør ikke noe tvilstilfelle – den skiller to ting som ikke
 * ligner på hverandre.
 */
function nullflateKant(d, px) {
  let n0 = 0;
  for (let i = 0; i < d.length; i++) if (d[i] === 0) n0++;
  if (!n0 || n0 === d.length) return;
  const naboer = [];
  const rader = Math.floor(d.length / px);
  for (let j = 0; j < rader; j++) {
    for (let i = 0; i < px; i++) {
      const k = j * px + i;
      if (d[k] !== 0) continue;
      if (i > 0 && d[k - 1] !== 0 && isFinite(d[k - 1])) naboer.push(Math.abs(d[k - 1]));
      if (i < px - 1 && d[k + 1] !== 0 && isFinite(d[k + 1])) naboer.push(Math.abs(d[k + 1]));
      if (j > 0 && d[k - px] !== 0 && isFinite(d[k - px])) naboer.push(Math.abs(d[k - px]));
      if (j < rader - 1 && d[k + px] !== 0 && isFinite(d[k + px])) naboer.push(Math.abs(d[k + px]));
    }
  }
  if (!naboer.length) return;
  naboer.sort((a, b) => a - b);
  /* 10-persentilen, ikke minimum: en dekningskant kan tilfeldigvis grense mot
     én celle nær null, og da ville minimum sagt «strandlinje» om et stup. */
  const p10 = naboer[Math.floor(naboer.length * 0.1)];
  if (p10 > 5) for (let i = 0; i < d.length; i++) if (d[i] === 0) d[i] = NaN;
}
...
module.exports = {
  ...
  lesGeoTiff, pakkFlis, pakkOpp, nullflateKant, hentFlis, hentBinært, hentJson, cacheMappe
};
